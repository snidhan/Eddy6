...
uZrms=np.load('uZrms.npy')
uZrmsPlane=np.mean(uZrms,axis=1)
...
for it, fileNumber in enumerate(listRes):
    ...
    tmp1 = np.abs(uZ-uZmean)
    tmp2 = np.mean((tmp1 - uZrmsPlane[:,np.newaxis,:])**2,axis=1)
        
    cv += tmp2**0.5/uZrmsPlane
    print('tpi:',time.time()-start)
    
    
cv/=nit

np.save('cv_uZrms.npy',cv)
